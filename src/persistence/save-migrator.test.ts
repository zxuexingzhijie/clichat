...
import { SaveDataV6Schema } from '../state/serializer';
...
describe('migrateToLatest', () => {
  ...
  it('validates V6 saves whose npc memory exceeds the legacy recent and salient limits', () => {
    const allMemories = Array.from({ length: 80 }, (_, index) => makeMemoryEntry(`memory-${index}`, index));
    const v6 = migrateToLatest({
      version: 5,
      meta: {
    ...
    const parseResult = SaveDataV6Schema.safeParse(v6);
